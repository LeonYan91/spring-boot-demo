    ...
    stages {
        ...
        stage('deploy') {
                    steps {
                        ...
                        sshPublisher(publishers: [sshPublisherDesc(configName: 'trainingserver', transfers: [sshTransfer(cleanRemote: false, excludes: '', execCommand: '', execTimeout: 120000, flatten: false, makeEmptyDirs: false, noDefaultExcludes: false, patternSeparator: '[, ]+', remoteDirectory: '/JENKINS_TMP', remoteDirectorySDF: false, removePrefix: 'target', sourceFiles: 'target/spring-boot-demo-0.0.1-SNAPSHOT.jar')], usePromotionTimestamp: false, useWorkspaceInPromotion: false, verbose: false)])
